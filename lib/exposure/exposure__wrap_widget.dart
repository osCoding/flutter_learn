...
class ExposureWrapWidget extends StatefulWidget {
  ExposureWrapWidget({
    Key key,
  ...
  State<StatefulWidget> createState() {
    return _ExposureWrapWidgetState();
  }
}

class _ExposureWrapWidgetState extends State<ExposureWrapWidget> {
  bool show = false;
  ScrollState state;
  ...
  void subscribeScrollNotification(BuildContext context) {
    final StreamController<ScrollNotification> publisher =
        ScrollNotificationPublish.of(context);
  ...
  void trackWidgetPosition(double scrollOffset, Axis direction) {
    ...
    final exposurePitSize =
        (context.findRenderObject() as RenderBox).size; // 当前widget 大小
    final viewPortSize =
        getViewPortSize(context) ?? const Size(1, 1); // 滚动可视区域大小
  ...
  double getExposureOffset(BuildContext context) {
    ...
    print("getExposureOffset-------->${offsetRevealToTop.offset}");
